...
public class GridLoc : Singleton<GridLoc> {
	...
	public void SetTileMap(){
		//grid.GameObject.enabled = true;
		Instantiate(grid, new Vector3(0,0,0), Quaternion.identity, gameObject.transform);
		tilemap = this.gameObject.transform.GetChild(0).GetChild(0).gameObject.GetComponent<Tilemap>();
		...
		foreach (var pos in tilemap.cellBounds.allPositionsWithin) {   
			...
			if (tilemap.HasTile(localPlace)) {
				tileWorldLocations.Add(tilemap.GetCellCenterLocal(localPlace));
			}
		}
